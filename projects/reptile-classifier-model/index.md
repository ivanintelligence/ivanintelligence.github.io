...
Our challenge is to solve an image classification problem using the CIFAR-100 dataset, specifically within the "Reptiles" superclass. We are tasked with developing a deep learning model capable of discerning the type of reptile depicted in an image. The "Reptiles" superclass comprises five subclasses, representing the target labels our model will learn to recognize through training.

Considering we have labeled data indicating the correct classification for each image, our approach is rooted in Supervised Learning. Furthermore, the task at hand is Classification, as our labels are categorical (e.g., crocodile, lizard, etc.), as opposed to Regression, which deals with continuous outputs.

<h2>Data Exploration</h2>

In this phase, we focus on acquiring the data necessary to train and evaluate our deep learning model. Specifically, we will utilize the "Reptiles" superclass from the CIFAR-100 dataset for this purpose.

Upon inspecting the CIFAR-100 dataset dictionary, which contains 20 superclasses, we identified that our work will only focus on one superclass, "Reptiles". "Reptiles" is the 16th superclass in the list, but since indexing starts from 0, it is indexed at 15. Hence, we have stated REPTILES_INDEX = 15 in our code.

Furthermore, we converted the index lists to NumPy arrays before extracting the "Reptiles" images and labels. This step enables us to quickly select the specific images of the superclass that we need and to work efficiently with the tools that assist us in building and training our machine learning model.

<h2>Architecture and Algorithm Selection</h2>

Given that we are addressing image classification problems, we rely on deep neural network architectures that are specifically designed for processing, analyzing, and making predictions from visual data. Among the various architectures available, Convolutional Neural Networks (CNNs) are recognized as the most effective and widely adopted solution for tasks related to images. CNNs excel at identifying patterns within images, such as edges, textures, and shapes, thanks to their unique structure that emulates the way the human visual cortex interprets visual information.
...
We further divided the data from the "Reptiles" superclass into three distinct sets: Training Set, Validation Set, and Test Set. This division serves multiple purposes:
...
Given that the CIFAR-100 dataset already segregates data into Training and Test Sets, we derived our Validation Set by allocating a percentage of the Training Set for this purpose. For our model, we allocated a Test Size of 0.2 or 20% of the 2,500 training images to form the Validation Set. This means our Validation Set comprises 500 images, matching the size of the Test Set, which already contains 500 images.

By partitioning our dataset in this manner, we establish a rigorous evaluation framework. It allows the model to be trained on one data subset, fine-tuned with another, and ultimately tested on unseen data. This approach ensures that the model's performance is not only effective on known instances but also generalizable to new data.

<h3>Feature Encoding</h3>

Our dataset encompasses five labels corresponding to the subclasses within the "Reptiles" superclass. To effectively utilize these categorical text labels for model training, they must be transformed into numerical values. This transformation is achieved through feature encoding.

While various feature encoding methods exist, we have opted for Label Encoding, wherein each unique category label is assigned a distinct integer.
...
Working with images entails dealing with pixel values that range from 0 to 255. Normalizing these values to fall between 0 and 1 aids our model's efficiency in understanding and processing the images. By scaling down our pixel values, we create a 'common ground' for all our images. This process is crucial as it enables our model to learn patterns and make predictions more quickly and accurately by mitigating potential confusion caused by the wide variations in pixel value magnitudes. Normalization is especially beneficial for the deep neural network architecture we plan to use, which will be discussed in subsequent phases.

<h2>Model Building</h2>

<h3>Convolution Layer</h3>

The Convolution Layer is the foundational building block of our CNN model. It executes a mathematical operation known as convolution. This layer utilizes filters (or kernels) that move across the input image (or the output from the preceding layer), producing feature maps in the process.
...
Following the convolutional stages, we encounter the Pooling Layer. This layer's main function is to diminish the spatial dimensions (width and height) of the input volume destined for subsequent convolution layers. It achieves this by summarizing the presence of features within patches of the feature map. This process aids in making the feature detection robust to changes in scale and orientation, while also lightening the computational load on the network.
...
There are two primary types of Pooling: Max Pooling and Average Pooling. We opt for Max Pooling, where the maximum value from each patch of the feature map is retained, and the rest are discarded. This approach contrasts with Average Pooling, which computes the average value of each patch on a feature map.
...
Pooling in our model is conducted over 2x2 windows with a stride of 2.

By repeatedly applying Convolutional and Pooling Layers, our network is capable of extracting increasingly complex and abstract features at each level. The initial layers may identify simple shapes or textures associated with reptiles, whereas the deeper layers are adept at recognizing entire objects or specific parts.

As we progress through our model, the number of filters in the Convolutional Layers is increased (from 64 to 128), enabling the network to learn more intricate features at each stage. This escalation is complemented by consistent use of 'same' padding and the ReLU activation function.
...
In our model, we configure the fully connected layer with 128 neurons and a ReLU activation function, which processes the flattened input to discern global patterns in the data. The final layer is another fully connected layer, this time with 5 neurons (one for each subclass within the "Reptiles" superclass), and employs the Softmax activation function to calculate probabilities for each class.

<h3>Regularization</h3>

One challenge that many machine learning models encounter, including ours, is overfitting. Overfitting occurs when a model learns the details and noise in the training data to such an extent that it adversely affects the model's performance on new, unseen data. An overfitted model becomes overly complex, capturing spurious patterns in the training data that do not generalize well.

To combat this issue, we implemented several key strategies.
...
We employed Dropout, a specific regularization technique that randomly "drops out" (i.e., sets to zero) a portion of neurons in the network during each training iteration. In our model, we set the Dropout rate to 0.5, meaning 50% of the inputs are randomly zeroed during training. This approach prevents neurons from co-adapting too closely to the training data, encouraging the network to learn more robust features that generalize better. Note that during model evaluation, Dropout is not applied, allowing the network to utilize all neurons.

Additionally, we used the EarlyStopping callback, which stops the training process if the model's performance on a validation dataset ceases to improve. We set EarlyStopping's patience to 5, indicating the training will halt if there's no improvement observed for 5 consecutive epochs. This technique's effectiveness will be highlighted in our Model Training section.

<h3>Optimization</h3>

Optimization algorithms adjust the weights of a neural network to minimize the loss function, which is a formula that measures the discrepancy between the model's predictions and the actual target values. This minimization improves our model's accuracy.

For the loss function, we chose Categorical Cross Entropy, suitable for multi-class classification tasks. It measures the dissimilarity between the true label distribution and our model's predictions. The Adam (Adaptive Moment Estimation) algorithm was selected for optimization due to its efficiency, combining elements from RMSprop and momentum to dynamically adjust the learning rate for each network weight based on the gradients' first and second moments. This enables Adam to find an optimal set of weights more effectively than many other optimization methods.

<h3>Hyperparameter Tuning</h3>

Through extensive experimentation and trial-and-error testing, we adjusted these hyperparameters to identify the optimal model performance for our project. This process led us to select the following hyperparameter values:
...
To enhance our model's robustness, we have integrated Data Augmentation into the Model Training process. This technique enriches the diversity of our training set by applying random, yet realistic, transformations such as rotation, shifting, and flipping.

For our model, we implemented the following transformations along with their specific parameters:
...
<div>
In our Model Evaluation (to be discussed in the subsequent phase), the total number of images in the Test Set is used to calculate the number of batches (or iterations) performed during its evaluation:
</div>
...
After training the model with the Training and Validation sets, we proceed to evaluate its performance using the Test set. This step is crucial for assessing how well our model generalizes to new, unseen data.

To visualize and better understand our model's performance, we plot these metrics. We use line graphs to compare Training and Validation losses and accuracies across multiple epochs. Additionally, we employ a bar graph to highlight the discrepancy between Training and Validation losses at their final epoch against the Test loss, as well as to show the difference between Training and Validation accuracies at their final epoch compared to the Test accuracy.

By generating line graphs for these metrics, we acquire Loss Curves and Accuracy Curves, which yield valuable insights into our model's learning behavior.

<figure style="--img-max: 320px;">
  <img src="/img/projects/reptile-classifier-model/loss-curve.jpg">
</figure>

The Loss Curve charts the changes in the loss function value throughout our training. The loss function measures the discrepancy between our model's predicted outputs and the actual target values. A descending loss curve signifies that our model is increasingly predicting more accurately over time.
...
Figure 1 illustrates our Training and Validation Losses, showcasing decreasing loss curves. This trend indicates that our model is learning to predict more accurately over time. Furthermore, the proximity of our training and validation losses, with minimal divergence, suggests that our model is effectively learning patterns that generalize beyond the training dataset, thereby indicating strong model performance.
